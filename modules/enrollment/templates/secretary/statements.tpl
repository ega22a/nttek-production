                            ...
                            <div role="tablist" id="accordion-<?php echo $counter; ?>" class="accordion">
                                <div class="card">
                                    ...
                                    <div class="collapse show item-1" role="tabpanel" data-parent="#accordion-<?php echo $counter; ?>">
                                        <div class="card-body">
                                            <?php $enrollies = $this -> database -> query("SELECT `id`, `lastname`, `firstname`, `patronymic`, `averageMark`, `isOnline`, `withStatement`, `withOriginalDiploma`, `degree`, `compositeKey`, `timestamp` FROM `enr_statements` WHERE `specialty` = {$row["id"]} AND `educationalType` = '" . (boolval($row["forExtramural"]) ? "extramural" : "fulltime") . "' AND `paysType` = 1 AND `isChecked` = 1 ORDER BY `averageMark` DESC;");
