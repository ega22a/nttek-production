                            ...
                            <div role="tablist" id="accordion-<?php echo $counter; ?>" class="accordion">
                                <div class="card">
                                    ...
                                    <div class="collapse show item-1" role="tabpanel" data-parent="#accordion-<?php echo $counter; ?>">
                                        <div class="card-body">
                                            <?php $enrollies = $this -> database -> query("SELECT `id`, `lastname`, `firstname`, `patronymic`, `averageMark`, `isOnline` FROM `enr_statements` WHERE `specialty` = {$row["id"]} AND `educationalType` = '" . (boolval($row["forExtramural"]) ? "extramural" : "fulltime") . "' AND `paysType` = 1 AND `isChecked` = 1 ORDER BY `averageMark` DESC;");
                                                ...
                                                while ($enrollee = $enrollies -> fetch_assoc()) {
                                                    ...
                                                    <?php } ?>
                                                        <li class="list-group-item d-flex justify-content-between align-items-center <?php echo $average == $enrollee["averageMark"] ? "bg-warning" : ""; ?>" data-id="<?php echo $enrollee["id"]; ?>">
                                                            <span><?php echo "{$crypt -> decrypt($enrollee["lastname"])} {$crypt -> decrypt($enrollee["firstname"])}" . (!empty($enrollee["patronymic"]) ? " " . $crypt -> decrypt($enrollee["patronymic"]) : "") . " ({$enrollee["averageMark"]})" . (boolval($enrollee["isOnline"]) ? "<i class=\"fas fa-cloud\" style=\"margin-left: 10px; cursor: default;\" data-toggle=\"tooltip\" data-bs-tooltip=\"\" type=\"button\" title=\"Онлайн-заявление\"></i>" : ""); ?></span>
