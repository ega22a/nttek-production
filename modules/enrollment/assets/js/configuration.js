...
function autofix(_type) {
    switch (_type) {
        case 1:
            createConfirm("<b>Настоятельно рекомендуется приостанавливать работу специалистов при запуске автофикса!</b><hr>Этот автофикс работает следующим образом: система автоматически восстанавливает дела, которые попали в проверку онлайн-заявлений, но они принимались лично. <b>ВНИМАНИЕ!</b> Запускайте эту функцию тогда и только тогда, когда все специалисты закончили принимать абитуриентов или только начали. Если специалист находится в процессе проверки заявления абитуриентом, то повремените. Этот процесс настоятельно рекомендуется запускать только тогда, когда специалисты распечатали расписки.", () => {
                $.post(
                    "../api/secretary/configuration/fix",
                    {
                        token: Cookies.get("token"),
                        fix: 1,
                    },
                    (data) => {
                        switch (data.status) {
                            case "OK":
                                createAlert("Операция прошла успешно! Можете продолжать работу.", "alert-success");
                            break;
                            default:
                                createAlert(`На сервере произошла ошибка. Подробнее: <b>${data.status}</b>.`, "alert-danger");
                            break;
        ...
        break;
        case 2:
            createConfirm("Этот автофикс исправляет ошибку, когда система пишет, что онлайн-заявление не приложено у тех абитуриентов, которые принесли оригинал документа об образовании. Полезный фикс, т.к. у абитуриентов пропадает форма загрузки Заявления.", () => {
                $.post(
                    "../api/secretary/configuration/fix",
                    {
                        token: Cookies.get("token"),
        ...
        break;
        default:
            createAlert(`Некорректный номер хотфикса!`, "alert-danger");
        break;
    }
}